...
3. Read `.local/<your-role>/SESSION.md` if it exists — resume from it.
...
**Workflow files** — at `.local/<your-role>/`, read by the human and other roles:
...
**Scratch space** — at `.local/<your-role>/scratch/`, free-form and private:
...
When your output is needed by another role, write `.local/<your-role>/HANDOFF.md`. The human reads it and routes to the next agent.
...
- Commit or stage any file under `.local/` — scratchpad is local-only and always gitignored. Using `git add -f` or `--force` to bypass `.gitignore` is forbidden.
